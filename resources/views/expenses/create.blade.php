@extends('layoutnew')
@section('title')
    {{trans_choice('general.add',1)}} {{trans_choice('general.new',1)}} {{trans_choice('general.expense',1)}}
@endsection
@section('content')
    <div class="box">
        ...
        {!! Form::open(array('url' => url('manager/expenses/create'), 'method' => 'post', 'name' => 'form',"enctype"=>"multipart/form-data")) !!}
        <div class="box-body">
            ...
            <div class="form-group">
                ...
                {!! Form::select('expense_type_id',$expense_types,null, array('class' => 'form-control select2', 'required'=>'required')) !!}
            </div>

            <div class="form-group">
                {!! Form::label('amount',trans_choice('general.amount',1),array('class'=>'')) !!}
                {!! Form::number('amount',null, array('class' => 'form-control','placeholder'=>trans_choice('general.amount',1),'required'=>'required','min'=>0)) !!}
            </div>

            <div class="form-group">
                {!! Form::label('status',trans_choice('general.status',1),array('class'=>'')) !!}
                {!! Form::select('status',array('pending'=>'Pending','completed'=>'Completed','canceled'=>'Canceled'),'completed', array('class' => 'form-control', 'placeholder'=>"Status",'required'=>'required')) !!}
            </div>

            <div class="form-group">
                {!! Form::label('comment',trans_choice('general.comment',2),array('class'=>'')) !!}
                {!! Form::textarea('comment',null, array('class' => 'form-control', 'placeholder'=>"Comments")) !!}
            </div>
        </div>
        <!-- /.box-body -->
