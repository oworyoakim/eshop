@extends('layoutnew')
@section('title')
    {{trans_choice('general.update',1)}} {{trans_choice('general.expense',1)}} {{trans_choice('general.type',1)}} {{trans_choice('general.information',1)}}
@endsection
@section('content')
    <div class="box">
        ...
        {!! Form::open(array('url' => url('manager/expenses/types/update/'.$expense_type->id), 'method' => 'post', 'name' => 'form',"enctype"=>"multipart/form-data")) !!}
        <div class="box-body">

            <div class="form-group">
                {!! Form::label('branch_id',trans_choice('general.branch',1),array('class'=>'')) !!}
                @if($branch_id)
                    {!! Form::select('branch_id',$branches,$branch_id, array('class' => 'form-control select2', 'required'=>'required','readonly'=>'readonly')) !!}
                @else
            ...
            <div class="form-group">
                ...
                {!! Form::text('title',$expense_type->title, array('class' => 'form-control','placeholder'=>trans_choice('general.title',1),'required'=>'required')) !!}
            </div>

            <div class="form-group">
                {!! Form::label('description',trans_choice('general.description',2),array('class'=>'')) !!}
                {!! Form::textarea('description',$expense_type->description, array('class' => 'form-control', 'placeholder'=>"Description")) !!}
            </div>
        </div>
        <!-- /.box-body -->
